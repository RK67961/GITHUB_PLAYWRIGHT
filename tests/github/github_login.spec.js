...
//   console.log('User is successfully logged in');
//     await page.context().storageState({ path:'playwright/.auth/github_loginstate.json' });
// });
test('verify user is sucessfully logged in with valid credentials user2',async({page})=>{
    // await page.goto('https://github.com/');
    // //await expect(page.getByRole('button',{name:'Sign in'})).toBeVisible();
    // //await page.getByRole('button',{name:'Sign in'}).click();
    // await page.locator(`(//a[contains(text(),'Sign in')])[2]`).click();
  ...
  await page.waitForURL('https://github.com/', { timeout: 20000 });
  console.log('User is successfully logged in');
    await page.context().storageState({ path:'playwright/.auth/github_loginstateuser2.json' });
});
